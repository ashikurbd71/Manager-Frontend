...
const CashinList = () => {
      ...
      try {
        ...
        if (active) {
          const res = await axoissecure.get(
            `/cashin/search?query=${search}&limit=${limit}&page=${page}`
          );
          setStat(res.data?.meta);
          return res?.data?.items;
        } else {
          const res = await axoissecure.get(
            `/cashin/search?limit=${limit}&page=${page}`
          );
          setStat(res.data?.meta);
          return res?.data?.items;
        }
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You want to delete this cash-in record!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          const res = await axoissecure.delete(`/cashin/${_id}`);
          if (res.status === 200) {
            Swal.fire({
              title: "Deleted!",
              text: "Cash-in record has been deleted successfully.",
              icon: "success",
            });
            refetch();
          }
          ...
          Swal.fire({
            title: "Error!",
            text: "Failed to delete cash-in record.",
            icon: "error",
          });
        }
      }
  ...
  const handleAllExport = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "All CashinList");

      XLSX.writeFile(workbook, "CashinList.xlsx");
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <div className="flex items-center space-x-2">
          <Link
            to={`/dashboard/updatecashin/${row.id}`}
          ...
          <Link
            to={`/dashboard/Cashininvoice/${row.id}`}
  ...
  const totalCashIn = items?.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  const totalTransactions = items?.length || 0;
  const averageAmount = totalTransactions > 0 ? (totalCashIn / totalTransactions).toFixed(2) : 0;
  ...
  return (
    <>
      <Helmet><title>Manager || Cash In List</title></Helmet>
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-secondary-900">Cash In List</h1>
              <p className="text-secondary-600">Manage all cash-in transactions</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-primary-900 mb-1">৳{totalCashIn.toLocaleString()}</h3>
          <p className="text-sm text-primary-600">Total Cash In</p>
        </ModernCard>

        <ModernCard className="text-center">
          <div className="w-12 h-12 bg-success-100 rounded-xl flex items-center justify-center mx-auto mb-3">
            <FaReceipt className="text-2xl text-success-600" />
          </div>
          <h3 className="text-2xl font-bold text-success-900 mb-1">{totalTransactions}</h3>
          <p className="text-sm text-success-600">Total Transactions</p>
        </ModernCard>
        ...
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-warning-900 mb-1">৳{averageAmount}</h3>
          <p className="text-sm text-warning-600">Average Amount</p>
        </ModernCard>

        <ModernCard className="text-center">
          <div className="w-12 h-12 bg-info-100 rounded-xl flex items-center justify-center mx-auto mb-3">
            <FaCalendarAlt className="text-2xl text-info-600" />
          </div>
          <h3 className="text-2xl font-bold text-info-900 mb-1">{todayTransactions}</h3>
          <p className="text-sm text-info-600">Today's Transactions</p>
        </ModernCard>
      </div>
      ...
      <ModernCard className="mb-6">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div className="search-input flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search cash-in records..."
              value={search}
      ...
      <ModernTable
        ...
        searchQuery={search}
        searchPlaceholder="Search cash-in records..."
        loading={isLoading}
        emptyMessage="No cash-in records found"
        className="mb-6"
      />
