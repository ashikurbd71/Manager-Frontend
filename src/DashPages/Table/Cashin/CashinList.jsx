...
console.log(items)
      ...
      Cell: ({ row }) => (
        <>
         <div className="flex w-full mx-auto  items-center gap-2 ">
          ...
          {/* View Icon */}
          <Link to={`/dashboard/Cashininvoice/${row.original.id}`}>
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axoissecure.delete(`/cashin/${_id}`).then((res) => {
          if (res.status === 200) {
            Swal.fire({
              title: "Deleted!",
  ...
  return (
    ...
    <h1 className="text-xl font-semibold text-[#0284C7] p-5">Cash In List</h1>

    <Tablenav handleExcell={handleAllExport} setActive={setActive} setSearch={setSearch} route={'/dashboard/addCashin'}/>
