...
const NoticeList = () => {
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axoissecure.delete(`/notice/${_id}`);
          if (res.status === 200) {
            Swal.fire({
              title: "Deleted!",
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value) => (
        <div className="flex items-center space-x-2">
          <FaFileAlt className="w-4 h-4 text-warning-500" />
          <span className="font-medium text-secondary-900 truncate max-w-xs">{value}</span>
        </div>
      )
    },
    {
      ...
      render: (value, row) => (
        <div className="flex items-center space-x-2">
          <Link
            to={`/dashboard/updatenotice/${row.id}`}
          ...
          <Link
            to={`/dashboard/detailsnotice/${row.id}`}
  ...
  const totalNotices = items?.length || 0;
  const todayNotices = items?.filter(item => {
    const today = new Date().toISOString().split('T')[0];
    return item.date?.split('T')[0] === today;
  }).length || 0;
  const thisWeekNotices = items?.filter(item => {
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
    return new Date(item.date) >= oneWeekAgo;
  }).length || 0;
  const thisMonthNotices = items?.filter(item => {
    ...
    <>
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-secondary-900">Notice List</h1>
              <p className="text-secondary-600">Manage all notices and announcements</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-primary-900 mb-1">{totalNotices}</h3>
        ...
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-success-900 mb-1">{todayNotices}</h3>
          <p className="text-sm text-success-600">Today's Notices</p>
        </ModernCard>
        ...
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-warning-900 mb-1">{thisWeekNotices}</h3>
          <p className="text-sm text-warning-600">This Week</p>
        </ModernCard>
        ...
        <ModernCard className="text-center">
          ...
          <h3 className="text-2xl font-bold text-info-900 mb-1">{thisMonthNotices}</h3>
          <p className="text-sm text-info-600">This Month</p>
        </ModernCard>
      </div>
