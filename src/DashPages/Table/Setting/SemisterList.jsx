...
import UpdateSemiter from '../../Update/SettingModal/UpdateSemiter';
      ...
      try {
        ...
        if (active) {
          const res = await axoissecure.get(
            `/semister/search?query=${search}&limit=${limit}&page=${page}`
          );
          ...
          const res = await axoissecure.get(
            `/semister/search?limit=${limit}&page=${page}`
          );
          setStat(res.data?.meta);
...
console.log(items)
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axoissecure.delete(`/semister/${_id}`).then((res) => {
          if (res.status === 200) {
            Swal.fire({
              title: "Deleted!",
  ...
  const handleDisable = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axoissecure.patch(`/semister/disable/${_id}`).then((res) => {
          if (res.status === 200) {
            Swal.fire({
              title: "Disabled!",
  ...
  const handleEnable = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axoissecure.patch(`/semister/enable/${_id}`).then((res) => {
          if (res.status === 200) {
            Swal.fire({
              title: "Enabled!",
  ...
  return (
    ...
    <Tablenav setActive={setActive} setSearch={setSearch} route={'/dashboard/setting/addsemister'}/>
